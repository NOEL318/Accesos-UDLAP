...
// header del módulo colegios con buscador, fecha, notificaciones y datos del oficial
export function ColegiosHeader({
  onMenuClick,
  searchPlaceholder = "Buscar residente, visitante o edificio...",
}: Props) {
  const { officer } = useColegiosSession()
  const now = new Date()
  const hora = now.toLocaleTimeString("es-MX", { hour: "2-digit", minute: "2-digit" })
  const fecha = now
    .toLocaleDateString("es-MX", { month: "short", day: "2-digit", year: "numeric" })
    .toUpperCase()
